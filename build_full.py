...
.slutspelnotis{display:flex;align-items:center;gap:13px;margin:0 0 16px;padding:13px 16px;border-radius:var(--r-sm);background:color-mix(in srgb,var(--gold) 14%,var(--card));border:1px solid color-mix(in srgb,var(--gold) 42%,transparent);box-shadow:var(--sh)}
.slutspelnotis .ssn-ic{font-size:22px;line-height:1;flex:none;animation:ssnpulse 2s var(--ease) infinite}
.slutspelnotis .ssn-txt{display:flex;flex-direction:column;gap:2px;min-width:0}
.slutspelnotis .ssn-txt strong{font-weight:800;font-size:14px;color:var(--ink);line-height:1.25}
.slutspelnotis .ssn-txt span{font-size:12px;color:var(--muted);font-weight:600}
@keyframes ssnpulse{0%,100%{opacity:1}50%{opacity:.4}}
@media(prefers-reduced-motion:reduce){.slutspelnotis .ssn-ic{animation:none}}
</style></head>
<body>
...
<div class="wrap">
  ...
  <!-- TEMP slutspel-notis: ta bort hela detta block (och .slutspelnotis-CSS) när slutspelsdatan finns -->
  <div class="slutspelnotis"><span class="ssn-ic">⏳</span><div class="ssn-txt"><strong>Väntar på uppdaterade uppgifter för slutspelet…</strong><span>Gruppspelet är färdigspelat – slutspelet fylls i inom kort.</span></div></div>
...
function renderChampViz(){const elC=document.getElementById("champviz"),elS=document.getElementById("skyviz");if(!elC||!elS)return;
  ...
  elC.innerHTML=section(champMap,true);elS.innerHTML=section(skyMap,false);
}
...
function openPerson(name){const p=PLIST.find(x=>x.name===name),pr=p.ref;
  ...
  document.getElementById("mBody").innerHTML=`<div class="statrow"><div class="stat"><div class="v">${p.pig} <span class="vp">poäng</span></div><div class="l">Gruppspel</div></div><div class="stat"><div class="v">${p.pis} <span class="vp">poäng</span></div><div class="l">Slutspel</div></div><div class="stat"><div class="v">${p.total} <span class="vp">poäng</span></div><div class="l">Total</div></div></div>${kboxHtml(pr)}<h4 style="margin:14px 0 6px">🎯 Gruppspel <span class="gpsub" style="font-weight:500;color:var(--muted);font-size:12px">(${g.exact+g.right} rätt tecken, varav ${g.exact} exakt \u00b7 ${g.gained} p på ${g.played} spelade)</span></h4><div class="psheet">${g.rows}</div>`;
  document.getElementById("overlay").classList.add("open");
}
...
function mapFeed(j){if(!j||!j.matches)return null;var alias={"Bosnia-Herzegovina":"Bosnien","Bosnien och Hercegovina":"Bosnien","Cape Verde Islands":"Kap Verde","Curaçao":"Curacao","Congo DR":"DR Kongo","Ecuador":"Equador"};var fx={};D.fixtures.forEach(function(f){fx[f.grp+"|"+f.home+"|"+f.away]=f.nr;});var out={},live={};j.matches.forEach(function(m){if(m.stage!=="GROUP_STAGE")return;var g=(m.group||"").replace("GROUP_","");var hn=alias[m.homeTeam]||m.homeTeam,an=alias[m.awayTeam]||m.awayTeam;var nr=fx[g+"|"+hn+"|"+an];if(nr==null)return;if(m.status==="IN_PLAY"||m.status==="PAUSED"){live[nr]=true;return;}if(m.status==="FINISHED"&&m.homeScore!=null&&m.awayScore!=null)out[nr]=[m.homeScore,m.awayScore];});return {group:out,updated:j.updatedAt||null,live:live};}
