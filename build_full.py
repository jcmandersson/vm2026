...
function picksGrouped(m){
  if(m.t!=="group"){
    ...
    return {sum:_home+' '+_gH.length+' · '+_away+' '+_gA.length+(_gBoth.length?' · lika '+_gBoth.length:''),html:_html};
  }
  const groups={};
...
function koJourneyHtml(ref,isMe){
  ...
  var finItems=(ref.final||[]).map(function(t){var c=((RESULTS.final||[]).indexOf(t)>=0&&!ks.elim[t])?'<span class="ksst win">i final</span>':aliveChip(t);return flag(t,14)+' '+t+' '+c;}).join(' <span class="ksr-sep">·</span> ');
...
var out={},live={},koFixtures={},ko={},koScore={},champion=null,rs={r32:{},r16:{},qf:{},sf:{},final:{}};
...
return {group:out,updated:j.updatedAt||null,live:live,koFixtures:koFixtures,ko:ko,koScore:koScore,r32:Object.keys(rs.r32),r16:Object.keys(rs.r16),qf:Object.keys(rs.qf),sf:Object.keys(rs.sf),final:Object.keys(rs.final),champion:champion};}
function applyResults(res){if(!res||!res.group)return;LIVE=res.live||{};Object.assign(RESULTS.group,res.group);if(res.koFixtures&&Object.keys(res.koFixtures).length)RESULTS.koFixtures=res.koFixtures;if(res.ko&&Object.keys(res.ko).length)RESULTS.ko=res.ko;["r32","r16","qf","sf","final"].forEach(function(k){if(res[k]&&res[k].length)RESULTS[k]=res[k];});if(res.champion)RESULTS.champion=res.champion;PLIST=buildPlist();MATCHES.forEach(function(m){if(m.t==="group"){m.score=RESULTS.group[m.nr]||null;}else{var _fx=RESULTS.koFixtures&&RESULTS.koFixtures[m.nr];m.home=(_fx&&_fx[0])||null;m.away=(_fx&&_fx[1])||null;m.score=(res.koScore&&res.koScore[m.nr])||null;}});cur=MATCHES.findIndex(function(m){return !m.score;});if(cur<0)cur=0;if(res.updated){var _f=fmtUpd(res.updated),_eu=document.getElementById("updatedAt");if(_f&&_eu)_eu.innerHTML="Senast uppdaterad "+_f;}renderPodium();renderFeatured();renderRank();renderMatchList();renderTables();renderChampViz();renderUpcoming();renderMeHem();renderH2H();renderBracket();renderTitleRace();}
