...
function renderFeatured(){
  ...
  var _fg=document.getElementById("fGuess");if(_fg){var meF=ME?D.participants.find(p=>p.name===ME):null;var pkF=(m.t==="group"&&!m.score&&meF&&meF._g[m.nr])?meF._g[m.nr]:null;var koFg="";if(m.t!=="group"&&meF&&(m.home||m.away)){var kpF=koPickTeam(meF,m);if(kpF&&kpF.team){var kwF=(RESULTS.ko||{})[m.nr];koFg='<span class="mxh-guess kog">Du tippade <b>'+kpF.team+(m.ph==="Final"?" som mästare":" vidare")+'</b>'+(kwF?(kwF===kpF.team?" ✓":" ✗"):"")+'</span>';}}_fg.innerHTML=pkF?'<span class="mxh-guess">Din gissning <b>'+pkF[0]+'–'+pkF[1]+'</b></span>':koFg;}
