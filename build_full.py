...
/* TABLES / BRACKET / H2H */
.grid2{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:13px}
...
.gtab table{width:100%;border-collapse:collapse;font-size:12.5px}.gtab td,.gtab th{padding:7px 8px;text-align:center;border-bottom:1px solid var(--line2)}
...
.hdr-main{position:relative;display:flex;align-items:center;gap:15px}
.ballwrap{position:relative;width:60px;height:66px;flex:0 0 auto}
.ball{position:absolute;top:2px;left:6px;width:48px;height:48px;animation:bob 1.5s cubic-bezier(.5,0,.5,1) infinite}
...
.hdr-foot{position:relative;margin-top:14px;display:flex;align-items:center;gap:7px;font-size:11.5px;font-weight:600;opacity:.95}
...
.mebanner .bn,.mebanner .bl,.mebanner .bpv,.mebanner .bps{text-shadow:0 1px 5px rgba(0,40,90,.4)}
...
.hdr h1{text-shadow:0 2px 10px rgba(0,0,0,.5);letter-spacing:-.4px}
...
.hdr h1,.bar h1{font-family:'Outfit',system-ui,sans-serif}
...
function renderPodium(){
  ...
  document.getElementById("podium").innerHTML=order.map((p,i)=>p?`<div class="pod ${cls[i]} ${p.name===ME?"me":""}" data-name="${p.name}"><span class="ava" style="background:${colorFor(p.name)}">${initials(p.name)}</span><span class="nm">${p.name}</span>${p.name===ME?'<span class="dut">DU</span>':""}<span class="pts">${p.total} poäng</span><div class="ped"><span class="ped-rank">${rank[i]}</span></div></div>`:'<div></div>').join("");
  document.getElementById("podium").querySelectorAll(".pod").forEach(el=>el.onclick=()=>openPerson(el.dataset.name));
  document.getElementById("podnote").textContent=anyPts?(PLIST[0].total>PLIST[1].total?`${PLIST[0].name} leder med ${PLIST[0].total-PLIST[1].total} poäng före tvåan`:`Delad ledning i toppen · ${PLIST[0].total} poäng`):"Turneringen har precis börjat – poängen tickar in efter varje match";
}
...
function renderTables(){document.getElementById("tablesWrap").innerHTML=Object.keys(D.groups).sort().map(L=>{const rows=groupTable(L);
  return `<div class="gtab"><h4>Grupp ${L}</h4><table><thead><tr><th>Lag</th><th>S</th><th>V</th><th>O</th><th>F</th><th>P</th></tr></thead><tbody>${rows.map((r,i)=>`<tr class="${i<2?'qual':''}"><td style="white-space:nowrap">${flag(r[0],14)} ${r[0]}</td><td>${r[1].P}</td><td>${r[1].W}</td><td>${r[1].D}</td><td>${r[1].L}</td><td><b>${r[1].Pts}</b></td></tr>`).join("")}</tbody></table></div>`;}).join("");}
