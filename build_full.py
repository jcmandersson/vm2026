...
.bar .tog{margin-left:auto;width:42px;height:42px;border-radius:13px;border:1px solid var(--line);background:var(--card);color:var(--ink);font-size:18px;cursor:pointer;transition:.3s var(--ease)}
...
.tab{flex:0 0 auto;border:1px solid var(--line);background:var(--card);color:var(--ink2);font-weight:700;font-size:13px;border-radius:999px;padding:9px 15px;cursor:pointer;transition:.25s var(--ease);color:var(--muted)}
...
.gtab th{font-size:10px;text-transform:uppercase;color:var(--muted)}.gtab td:first-child,.gtab th:first-child{text-align:left;padding-left:10px}
.gtab th:nth-child(n+2),.gtab td:nth-child(n+2){width:18px}
...
.gtlegend{display:flex;flex-wrap:wrap;gap:5px 14px;margin-top:14px;font-size:11.5px;color:var(--muted)}
.gtlegend b{color:var(--ink);font-weight:800;margin-right:3px}
.bt-intro{font-size:13px;color:var(--muted);margin:0 0 12px;line-height:1.5}
.bt-row{display:flex;align-items:center;gap:9px;padding:8px 10px;border-radius:11px;background:var(--soft);border:1px solid var(--line2);font-size:13px;margin-bottom:6px}
.bt-row.q{background:color-mix(in srgb,var(--grass) 12%,transparent);border-color:color-mix(in srgb,var(--grass) 22%,transparent)}
.bt-row.q .bt-rank{color:var(--grass-deep)}
.bt-row.out{opacity:.58}
.bt-rank{width:16px;text-align:center;font-weight:800;color:var(--muted);font-size:12px;flex:0 0 auto}
.bt-grp{width:20px;height:20px;border-radius:6px;background:var(--soft2);display:grid;place-items:center;font-weight:800;font-size:10.5px;color:var(--muted);flex:0 0 auto}
.bt-team{flex:1;min-width:0;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.bt-stat{font-weight:800;font-size:12px;color:var(--muted);flex:0 0 auto}
.bt-gd{min-width:30px;text-align:right}
.bt-cut{display:flex;align-items:center;gap:8px;margin:10px 2px;font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:var(--grass-deep);font-weight:800}
.bt-cut::before,.bt-cut::after{content:"";flex:1;height:1px;background:color-mix(in srgb,var(--grass) 38%,transparent)}
...
/* MODAL */
.overlay{position:fixed;inset:0;background:rgba(5,10,25,.6);display:none;place-items:center;z-index:60;padding:14px;animation:rise .25s var(--ease)}
...
#mBody .ps.pshead{position:sticky;top:0;z-index:2;background:var(--bg);padding:5px 10px;box-shadow:0 6px 7px -6px rgba(0,10,40,.15)}
@media(max-width:560px){.gpsub{display:block;margin-top:3px}}
...
/* sticky flikrad */
.tabs{position:sticky;top:0;z-index:30;margin:0 -16px 16px;padding:9px 16px;background:var(--bg)}
...
.brow .chk{margin-left:auto;color:var(--blue);font-weight:900;font-size:11.5px;white-space:nowrap;padding-right:6px}
...
.ksremain{font-size:12px;font-weight:700;color:var(--muted);margin:-3px 0 10px}.ksremain b{color:var(--blue)}
.ksresa{display:flex;flex-direction:column;gap:7px;margin:0 0 11px;padding-bottom:11px;border-bottom:1px solid var(--line2)}
.ksr-item{display:flex;align-items:flex-start;gap:10px;font-size:12.5px}.ksr-l{font-weight:800;color:var(--ink2);min-width:92px;flex:none}
.ksr-v{font-weight:700;display:flex;align-items:center;gap:6px;flex-wrap:wrap;color:var(--ink)}.ksr-sep{color:var(--muted)}
.ksst{font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.4px;border-radius:999px;padding:2px 8px;white-space:nowrap}
.ksst.alive{background:color-mix(in srgb,var(--grass) 20%,transparent);color:var(--grass-deep)}
.ksst.out{background:color-mix(in srgb,var(--down) 15%,transparent);color:var(--down)}
.ksst.win{background:color-mix(in srgb,var(--gold) 24%,transparent);color:var(--yellow-deep)}
...
.hdr-main{position:relative;z-index:2;display:flex;align-items:center;gap:15px;padding-right:44px}
.ballwrap{position:relative;width:60px;height:66px;flex:0 0 auto;-webkit-tap-highlight-color:transparent;touch-action:manipulation}
.ball{position:absolute;top:2px;left:6px;width:48px;height:48px;animation:bob 1.5s cubic-bezier(.5,0,.5,1) infinite;-webkit-tap-highlight-color:transparent;-webkit-touch-callout:none;user-select:none;-webkit-user-select:none;touch-action:manipulation}
...
.hdr .tog{position:absolute;top:13px;right:13px;z-index:3;width:34px;height:34px;border-radius:11px;border:1px solid rgba(255,255,255,.2);background:rgba(255,255,255,.16);color:#fff;opacity:.82;cursor:pointer;transition:.25s var(--ease)}
@media(hover:hover){.hdr .tog:hover{opacity:1;background:rgba(255,255,255,.24)}}
.hdr .tog{display:grid;place-items:center}.hdr .tog svg{width:18px;height:18px}
...
@media(max-width:560px){.hdr h1{font-size:29px}.hdr{padding:17px 16px 13px}}
.hdr-foot{flex-wrap:wrap}
.hdr-me{margin-left:auto;display:inline-flex;align-items:center;gap:7px;font-weight:700}
.hdr select{background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.32);color:#fff;border-radius:9px;padding:5px 10px;font-weight:700;font-size:12.5px;font-family:inherit;cursor:pointer;min-width:auto;flex:0 0 auto}
...
.msel-btn{display:inline-flex;align-items:center;gap:7px;background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.32);color:#fff;border-radius:11px;padding:5px 10px 5px 6px;font-weight:700;font-size:12.5px;cursor:pointer;transition:.2s var(--ease);font-family:inherit;max-width:200px}
...
.hdr .tog{border-color:rgba(255,255,255,.22);background:rgba(255,255,255,.12)}
...
@media(max-width:560px){
 .tabs{position:fixed;top:auto;bottom:0;left:0;right:0;margin:0;padding:6px 4px;gap:0;justify-content:space-around;background:var(--bg);border-top:1px solid var(--line);border-radius:0;overflow:visible;z-index:60}
...
.kbnote{font-size:11px;color:var(--muted);margin:5px 2px 0;font-style:italic}
...
.mdaygroup{background:var(--card);border:1px solid var(--line);border-radius:var(--r);box-shadow:var(--sh);padding:9px 15px 6px;margin-bottom:14px}
...
.cv .scoregrp{align-items:center;gap:12px}
.cv .rchip{flex:0 0 170px;width:170px;justify-content:flex-start;background:none;border:none;padding:0}
.cv .cvk{font-weight:800;font-size:14px}
.cv .rchip .cvk{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.cv .ppl{flex:1}
.cv .cntr{flex:0 0 auto;width:44px;text-align:right}
@media(max-width:560px){.cv .scoregrp{flex-wrap:wrap;row-gap:9px;align-items:center}.cv .rchip{flex:0 0 auto;width:auto}.cv .ppl{flex-basis:100%}}
...
.podnote .nmg,.podnote .nms,.podnote .nmb{white-space:nowrap}
.podnote .nmg{color:#b8860b}.podnote .nms{color:#67788f}.podnote .nmb{color:#a86a36}
[data-theme=dark] .podnote .nmg{color:var(--gold)}[data-theme=dark] .podnote .nms{color:var(--silver)}[data-theme=dark] .podnote .nmb{color:var(--bronze)}
...
.mdaygroup .mday{position:sticky;top:6px;z-index:6;background:var(--card);padding:7px 8px;margin:0 -6px 6px;border-radius:9px}
...
@media(max-width:560px){
 [data-theme=light] .tabs{background:#0b1530;border-top:1px solid rgba(255,255,255,.10)}
 [data-theme=light] .tab{color:rgba(234,240,255,.6)}
 [data-theme=light] .tab.active{color:#fff}
 [data-theme=dark] .tabs{background:#eef2fb;border-top:1px solid rgba(11,21,48,.10)}
...
.mebar{display:none;align-items:center;margin:0 0 16px}
.mebar .hdr-me{width:100%;margin:0;display:flex;align-items:center;flex-wrap:wrap;gap:9px}
.mebar #meLabel{font-size:13px;font-weight:700;color:var(--muted);order:-1;flex-basis:100%;margin:0}
.mebar .msel{flex:1;min-width:0}
.mebar .msel-btn{width:100%;max-width:none;justify-content:flex-start;background:var(--card);border:1px solid var(--line);color:var(--ink);backdrop-filter:none;-webkit-backdrop-filter:none;box-shadow:var(--shs);padding:9px 12px;border-radius:12px;font-size:13.5px}
.mebar .msel-btn:hover{background:var(--soft2)}
.mebar .meprof{background:var(--card);border:1px solid var(--line);color:var(--ink);width:42px;height:42px;border-radius:12px;box-shadow:var(--shs)}
.mebar .meprof:hover{background:var(--soft2);transform:translateY(-1px)}
...
.mxh-guess.kog{gap:6px}.mxh-guess.kog b{font-size:14.5px}
.mxh-guess.rt{color:var(--grass-deep);background:color-mix(in srgb,var(--grass) 14%,transparent);border-color:color-mix(in srgb,var(--grass) 34%,transparent)}
.mxh-guess.wr{color:#a8102b;background:color-mix(in srgb,var(--down) 12%,transparent);border-color:color-mix(in srgb,var(--down) 30%,transparent)}
[data-theme=dark] .mxh-guess.wr{color:#ff8a9c}
[data-theme=dark] .mxh-guess.rt{color:var(--grass)}
[data-theme=dark] .mx-guess.wrong .gv,[data-theme=dark] .mx-guess.wrong .gl{color:#ff8a9c}
[data-theme=dark] .mx-guess.right .gv,[data-theme=dark] .mx-guess.right .gl{color:#ffb454}
[data-theme=dark] .mx-guess.exact .gv,[data-theme=dark] .mx-guess.exact .gl{color:var(--grass)}
...
<div class="wrap">
  ...
  <section class="view active" id="dash">
    ...
    <div class="duo">
      ...
      <div class="card duocard"><div class="ttl"><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="17" rx="2"/><path d="M3 9h18M8 2v4M16 2v4"/></svg></span>Kommande matcher</div><div id="upcoming"></div></div>
    </div>
    <div class="mgroup">
  ...
  <section class="view" id="standings"><div class="card" id="titleRaceCard" style="display:none"><div class="ttl">🏆 Titelracet <span class="more" id="trSub"></span></div><div id="titleRace"></div></div><div class="card"><div class="ttl"><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 4h12v3a6 6 0 0 1-12 0V4z"/><path d="M6 5H3v2a3 3 0 0 0 3 3"/><path d="M18 5h3v2a3 3 0 0 1-3 3"/><path d="M9 18h6M12 14v4"/></svg></span>Poänglista <span class="more" id="partCount"></span><button class="infobtn" id="poengInfo">ⓘ Så funkar poängen</button></div><div class="rk-ctrl"><input id="rkSearch" type="search" placeholder="Sök deltagare…" aria-label="Sök deltagare"><div class="rk-sort" id="rkSort"><button data-k="total" class="active">Total</button><button data-k="pig">Gruppspel</button><button data-k="pis">Slutspel</button></div></div><div id="rankList"></div>
    ...
    <div class="card"><div class="ttl"><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 4h12v3a6 6 0 0 1-12 0V4z"/><path d="M6 5H3v2a3 3 0 0 0 3 3"/><path d="M18 5h3v2a3 3 0 0 1-3 3"/><path d="M9 18h6M12 14v4"/></svg></span>Tippade världsmästare</div><div class="cv" id="champviz"></div></div>
    <div class="card"><div class="ttl"><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1.5"/></svg></span>Tippade skyttekung</div><div class="cv" id="skyviz"></div></div>
  </section>
...
<div class="kogrid" id="bracketWrap"></div></div></section>
  <section class="view" id="tables"><div class="card"><div class="ttl"><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/></svg></span>Grupptabeller · faktiska resultat</div><div class="grid2" id="tablesWrap"></div><div class="gtlegend"><span><b>S</b> Spelade</span><span><b>V</b> Vunna</span><span><b>O</b> Oavgjorda</span><span><b>F</b> Förlorade</span><span><b>GM</b> Gjorda mål</span><span><b>IM</b> Insläppta mål</span><span><b>MS</b> Målskillnad</span><span><b>P</b> Poäng</span></div></div><div class="card"><div class="ttl"><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 4h12v3a6 6 0 0 1-12 0V4z"/><path d="M6 5H3v2a3 3 0 0 0 3 3"/><path d="M18 5h3v2a3 3 0 0 1-3 3"/><path d="M9 18h6M12 14v4"/></svg></span>Kampen om slutspelet <span class="more">Avgjort ✅</span></div><p class="bt-intro">Utöver gruppettorna och grupptvåorna går de <b>8 bästa grupptreorna</b> vidare till slutspelet. Så ligger treorna till just nu – de över strecket är på väg vidare:</p><div id="bestThirds"></div></div></section>
...
const MATCHES=D.fixtures.map(f=>({t:"group",ph:"Grupp "+f.grp,nr:f.nr,home:f.home,away:f.away,time:f.date,score:RESULTS.group[f.nr]||null})).concat(D.knockout.slice().sort((a,b)=>{var ka=parseKick(a.date),kb=parseKick(b.date);return (ka?ka.getTime():0)-(kb?kb.getTime():0);}).map(k=>({t:"ko",ph:k.round,nr:k.nr,home:null,away:null,homeSlot:k.homeSlot,awaySlot:k.awaySlot,time:k.date,score:null})));
MATCHES.sort((a,b)=>{const A=parseKick(a.time),B=parseKick(b.time);return (A?+A:8.64e15)-(B?+B:8.64e15);});
let cur=MATCHES.findIndex(m=>!m.score);if(cur<0)cur=0;
let mFilter="all";
let rankSort="total",rankQ="",trOpen=false;
...
function pick(v){ME=v||null;try{ME?localStorage.setItem("loftahammar_me",ME):localStorage.removeItem("loftahammar_me");}catch(e){}close();label();build();renderMeHem();renderPodium();renderRank();renderFeatured();renderMatchList();renderBracket();renderTitleRace();}
...
function openPop(){const r=btn.getBoundingClientRect();pop.style.top=(r.bottom+8)+"px";if(r.width>=260){pop.style.left=r.left+"px";pop.style.right="auto";pop.style.width=r.width+"px";pop.style.minWidth=r.width+"px";}else{pop.style.right=Math.max(8,window.innerWidth-r.right)+"px";pop.style.left="auto";pop.style.width="";pop.style.minWidth="";}dd.classList.add("open");pop.classList.add("open");}
...
function koState(){var inR32=RESULTS.r32||[],koF=RESULTS.koFixtures||{},koW=RESULTS.ko||{},elim={};Object.keys(koW).forEach(function(nr){var fx=koF[nr];if(!fx)return;var w=koW[nr];[fx[0],fx[1]].forEach(function(t){if(t&&t!==w)elim[t]=1;});});return {started:inR32.length>0,elim:elim,alive:function(t){if(!t)return false;if(!inR32.length)return true;return inR32.indexOf(t)>=0&&!elim[t];}};}
function koRemain(ref,ks){var s=0;[["r16",3],["qf",5],["sf",7],["final",8]].forEach(function(x){s+=(ref[x[0]]||[]).filter(function(t){return ks.alive(t)&&(RESULTS[x[0]]||[]).indexOf(t)<0;}).length*x[1];});if(!RESULTS.champion&&ks.alive(ref.champion))s+=10;return s;}
function koPickTeam(ref,m){var d=function(t){if(!t)return -1;if(ref.champion===t)return 5;if((ref.final||[]).indexOf(t)>=0)return 4;if((ref.sf||[]).indexOf(t)>=0)return 3;if((ref.qf||[]).indexOf(t)>=0)return 2;if((ref.r16||[]).indexOf(t)>=0)return 1;return 0;};var dh=d(m.home),da=d(m.away);if(dh<=0&&da<=0)return null;if(dh===da)return {team:null};return {team:dh>da?m.home:m.away};}
...
function renderPodium(){
  ...
  if(!anyPts()){_pn="Turneringen har precis börjat – poängen tickar in efter varje match";}
    ...
    if(_tie){var _ns=_tie.names,_pl=_tie.place;if(_ns.length>3){_pn=_ns.length+" spelare delar "+_pl+":a platsen med "+_tie.total+" poäng vardera.";}else{var _c=_ns.map(function(n){return _mn(n,_pl);}),_who=_c.length===2?_c[0]+" och "+_c[1]:_c.slice(0,-1).join(", ")+" och "+_c[_c.length-1];_pn=_who+" ligger på delad "+_pl+":a plats med "+_tie.total+" poäng vardera.";}}
    else{_pn=_mn(PLIST[0].name,1)+" leder med "+(PLIST[0].total-PLIST[1].total)+" poäng före tvåan";}
  }
  document.getElementById("podnote").innerHTML=_pn;
}
function parseKick(t){const m=/(\d{1,2})\/(\d{1,2})\s+(\d{1,2})[.:](\d{2})/.exec(t||"");if(!m)return null;return new Date(2026,+m[2]-1,+m[1],+m[3],+m[4]);}
function setupCountdown(m){var el=document.getElementById("fCd");if(!el)return;if(window._cd)clearInterval(window._cd);var nx=MATCHES.findIndex(function(x){return !x.score;}),_nm=(nx>=0&&MATCHES[nx]===m);if(!m.score&&_nm&&isLive(m)){el.style.display="";el.innerHTML='<div class="mxh-live"><span class="mxh-livedot"></span>Inväntar resultat</div>';return;}var k=(!m.score&&_nm)?parseKick(m.time):null;if(!k){el.innerHTML="";el.style.display="none";return;}el.style.display="";var _seg=function(n,l){return '<span class="cdu"><span class="cdn">'+("0"+n).slice(-2)+'</span><span class="cdt">'+l+'</span></span>';};function tick(){var d=k-Date.now();if(d<=0){el.innerHTML='<div class="cdlive">🟢 Avspark passerad – inväntar resultat</div>';if(window._cd)clearInterval(window._cd);return;}var dd=Math.floor(d/864e5),hh=Math.floor(d%864e5/36e5),mm=Math.floor(d%36e5/6e4),ss=Math.floor(d%6e4/1e3);el.innerHTML='<div class="cdtop">Avspark om</div><div class="cdrow">'+(dd>0?_seg(dd,"DGR")+'<span class="cdsep">:</span>':"")+_seg(hh,"TIM")+'<span class="cdsep">:</span>'+_seg(mm,"MIN")+'<span class="cdsep">:</span>'+_seg(ss,"SEK")+'</div>';}tick();window._cd=setInterval(tick,1000);}
...
function renderUpcoming(){const el=document.getElementById("upcoming");if(!el)return;const up=[];for(let i=0;i<MATCHES.length&&up.length<7;i++){if(!MATCHES[i].score)up.push(i);}
  ...
  el.innerHTML=up.map(i=>{const m=MATCHES[i];const pr=(m.time||"").split(" ");const teams=(m.home||m.away)?`${m.home?flag(m.home,15):""} <span class="un">${m.home||m.homeSlot}</span> <span class="uvs">–</span> <span class="un">${m.away||m.awaySlot}</span> ${m.away?flag(m.away,15):""}`:`<span class="uslot">${m.homeSlot} – ${m.awaySlot}</span>`;return `<div class="uprow" data-i="${i}"><div class="utime">${pr[0]||""}<span>${pr[1]||""}</span></div><div class="ubody"><div class="uteams">${teams}</div><div class="utag">${m.ph}</div></div></div>`;}).join("");
...
function renderChampViz(){const elC=document.getElementById("champviz"),elS=document.getElementById("skyviz");if(!elC||!elS)return;
  ...
  const ks=koState();
  const section=(map,withFlag,markElim)=>{const arr=Object.entries(map).sort((a,b)=>b[1].length-a[1].length);return arr.map(([k,names])=>{const dead=markElim&&ks.started&&!ks.alive(k);return `<div class="scoregrp${dead?' cv-dead':''}"><span class="rchip">${withFlag?flag(k,15):""}<span class="cvk">${k}</span>${dead?'<span class="ksst out">utslagna</span>':""}</span><div class="ppl">${chips(names)}</div><span class="cntr">${names.length} st</span></div>`;}).join("");};
  elC.innerHTML=section(champMap,true,true);elS.innerHTML=section(skyMap,false,false);
}
function meBannerHtml(me){if(!me)return "";var ks=koState();var mx=ks.started?(me.total+koRemain(me.ref,ks)):494;return `<div class="mebanner" data-name="${me.name}" style="cursor:pointer" title="Visa min tipslapp"><span class="ava">${initials(me.name)}</span><div class="bi"><div class="bl">⭐ Din ställning</div><div class="bn">${me.name} · plats #${me.rank} av ${PLIST.length}</div><div class="mebar2"><div class="mefill" style="width:${(me.total/494*100).toFixed(1)}%"></div></div></div><div class="bp"><div class="bpv">${me.total}</div><div class="bpu">poäng</div><div class="bps">${ks.started?"max "+mx+" p":"av 494"}</div></div></div>`;}
...
function renderMeHem(){updMeProf();const el=document.getElementById("meHem");if(el)el.innerHTML=ME?meBannerHtml(PLIST.find(p=>p.name===ME)):'<div class="mehint">👋 Välj ditt namn ovan för en personlig vy - Se din placering, dina gissningar och dina poäng.</div>';}
function renderFeatured(){
  ...
  if(m.home||m.away){
    ...
    document.getElementById("hName").textContent=m.home||m.homeSlot||"TBD";document.getElementById("aName").textContent=m.away||m.awaySlot||"TBD";
    if(m.score){sc.textContent=`${m.score[0]} – ${m.score[1]}`;sc.className="sc pop";st.textContent="Slutresultat";}
    else{sc.textContent="VS";sc.className="sc pend pop";st.textContent=m.t==="group"?"Kommande":m.ph;}
  }else{
  ...
  var _fg=document.getElementById("fGuess");if(_fg){var meF=ME?D.participants.find(p=>p.name===ME):null;var pkF=(m.t==="group"&&!m.score&&meF&&meF._g[m.nr])?meF._g[m.nr]:null;var koFg="";if(m.t!=="group"&&meF&&(m.home||m.away)){var kpF=koPickTeam(meF,m);if(kpF&&kpF.team){var kwF=(RESULTS.ko||{})[m.nr];koFg='<span class="mxh-guess kog'+(kwF?(kwF===kpF.team?" rt":" wr"):"")+'">Du tippade <b>'+kpF.team+(m.ph==="Final"?" som mästare":" vidare")+'</b>'+(kwF?(kwF===kpF.team?" ✓":" ✗"):"")+'</span>';}}_fg.innerHTML=pkF?'<span class="mxh-guess">Din gissning <b>'+pkF[0]+'–'+pkF[1]+'</b></span>':koFg;}
...
function picksGrouped(m){
  if(m.t!=="group"){
    var _home=m.home,_away=m.away;
    if(!_home&&!_away)return {sum:"",html:'<div style="color:var(--muted);font-size:12.5px;padding:6px">Lagen avgörs när de föregående matcherna är spelade.</div>'};
    var _lbl=(m.ph==="Final")?" som mästare":" vidare";
    var _depth=function(ref,t){if(!t)return -1;if(ref.champion===t)return 5;if((ref.final||[]).indexOf(t)>=0)return 4;if((ref.sf||[]).indexOf(t)>=0)return 3;if((ref.qf||[]).indexOf(t)>=0)return 2;if((ref.r16||[]).indexOf(t)>=0)return 1;return 0;};
    var _gH=[],_gA=[],_gBoth=[],_none=0;
    PLIST.forEach(function(pp){var dh=_depth(pp.ref,_home),da=_depth(pp.ref,_away);if(dh<=0&&da<=0){_none++;return;}if(dh>da)_gH.push(pp.name);else if(da>dh)_gA.push(pp.name);else _gBoth.push(pp.name);});
    var _chip=function(n){return '<span class="pp2 '+(n===ME?"me":"")+'" data-name="'+n+'"><span class="ava" style="background:'+colorFor(n)+'">'+initials(n)+'</span>'+first(n)+'</span>';};
    var _block=function(flagH,label,names){if(!names.length)return "";return '<div class="scoregrp"><span class="rchip">'+flagH+'<span class="rsc">'+label+'</span></span><div class="ppl">'+names.map(_chip).join("")+'</div><span class="cntr">'+names.length+' st</span></div>';};
    var _html='<div class="catblock">'
      +_block('<span class="wf">'+flag(_home,15)+'</span>',_home+_lbl,_gH)
      +_block('<span class="wf">'+flag(_away,15)+'</span>',_away+_lbl,_gA)
      +_block('<span class="wf draw">=</span>','Ingen favorit',_gBoth)
      +'</div>';
    if(_gBoth.length)_html+='<div class="kbnote">"Ingen favorit" = tippade båda lagen exakt lika långt i sitt slutspelsträd.</div>';
    if(_none)_html+='<div class="kbnote">'+_none+' hade inget av lagen kvar så här långt.</div>';
    var _sumParts=[];if(_home)_sumParts.push(_home+' '+_gH.length);if(_away)_sumParts.push(_away+' '+_gA.length);if(_gBoth.length)_sumParts.push('ingen favorit '+_gBoth.length);
    return {sum:_sumParts.join(' · '),html:_html};
  }
  const groups={};
...
function openMatch(i){const m=MATCHES[i];const r=picksGrouped(m);const av=document.getElementById("mAva");av.style.display="none";
  var _hN=m.home||m.homeSlot,_aN=m.away||m.awaySlot;document.getElementById("mName").textContent=m.score?`${_hN} ${m.score[0]}–${m.score[1]} ${_aN}`:`${_hN} – ${_aN}`;
...
var _trKey=null,_trRows=null;
function computeTitleRace(){
  var inR32=RESULTS.r32||[];if(!inR32.length){_trRows=null;_trKey="none";return null;}
  var key=JSON.stringify([RESULTS.group,RESULTS.r32,RESULTS.r16,RESULTS.qf,RESULTS.sf,RESULTS.final,RESULTS.champion,RESULTS.ko,RESULTS.koFixtures,PLIST.length]);
  if(key===_trKey&&_trRows)return _trRows;
  var koF=RESULTS.koFixtures||{},koW=RESULTS.ko||{};
  var tid={},teams=[];var id=function(t){if(t==null)return -1;if(tid[t]==null){tid[t]=teams.length;teams.push(t);}return tid[t];};
  inR32.forEach(function(t){id(t);});
  var lvl={},feed={},koWId={};
  D.knockout.forEach(function(k){lvl[k.nr]=(k.round==="16-delsfinal"?1:k.round==="Åttondelsfinal"?2:k.round==="Kvartsfinal"?3:k.round==="Semifinal"?4:5);
    if(k.round==="16-delsfinal"){var fx=koF[k.nr]||[];feed[k.nr]={team:true,a:id(fx[0]),b:id(fx[1])};}
    else{var ma=/Match nr ([0-9]+)/.exec(k.homeSlot||""),mb=/Match nr ([0-9]+)/.exec(k.awaySlot||"");feed[k.nr]={team:false,a:ma?+ma[1]:0,b:mb?+mb[1]:0};}
    koWId[k.nr]=(koW[k.nr]!=null?id(koW[k.nr]):-1);});
  var order=Object.keys(feed).map(Number).sort(function(a,b){return a-b;});
  var K=teams.length;
  var pls=PLIST.map(function(p){var mp=function(arr){var o=[];(arr||[]).forEach(function(t){var d=tid[t];if(d!=null)o.push(d);});return o;};
    return {name:p.name,pig:p.pig,total:p.total,base:p.pig+2*inter(p.ref.r32,inR32),r16:mp(p.ref.r16),qf:mp(p.ref.qf),sf:mp(p.ref.sf),fin:mp(p.ref.final),ch:(tid[p.ref.champion]!=null?tid[p.ref.champion]:-1)};});
  var s=1234567,rng=function(){s=s+0x6D2B79F5|0;var t=Math.imul(s^s>>>15,1|s);t=t+Math.imul(t^t>>>7,61|t)^t;return ((t^t>>>14)>>>0)/4294967296;};
  var N=50000,L=pls.length,c1=new Int32Array(L),c2=new Int32Array(L),c3=new Int32Array(L);
  var reach=new Int8Array(K),winId=new Int16Array(104),tot=new Float64Array(L),oi,pi,a,e;
  for(var it=0;it<N;it++){
    reach.fill(0);
    for(oi=0;oi<order.length;oi++){var nr=order[oi],f=feed[nr],A=f.team?f.a:winId[f.a],B=f.team?f.b:winId[f.b],w;
      if(koWId[nr]>=0)w=koWId[nr];else if(A<0)w=B;else if(B<0)w=A;else w=rng()<0.5?A:B;winId[nr]=w;if(w>=0)reach[w]=lvl[nr];}
    for(pi=0;pi<L;pi++){var p=pls[pi],v=p.base,r=p.r16,q=p.qf,sf=p.sf,fn=p.fin;
      for(a=0;a<r.length;a++){if(reach[r[a]]>=1)v+=3;}
      for(a=0;a<q.length;a++){if(reach[q[a]]>=2)v+=5;}
      for(a=0;a<sf.length;a++){if(reach[sf[a]]>=3)v+=7;}
      for(a=0;a<fn.length;a++){if(reach[fn[a]]>=4)v+=8;}
      if(p.ch>=0&&reach[p.ch]>=5)v+=10;
      tot[pi]=v;}
    var i1=-1,i2=-1,i3=-1;
    for(pi=0;pi<L;pi++){var tv=tot[pi],pg=pls[pi].pig;
      if(i1<0||tv>tot[i1]||(tv===tot[i1]&&(pg>pls[i1].pig||(pg===pls[i1].pig&&pls[pi].name<pls[i1].name)))){i3=i2;i2=i1;i1=pi;}
      else if(i2<0||tv>tot[i2]||(tv===tot[i2]&&(pg>pls[i2].pig||(pg===pls[i2].pig&&pls[pi].name<pls[i2].name)))){i3=i2;i2=pi;}
      else if(i3<0||tv>tot[i3]||(tv===tot[i3]&&(pg>pls[i3].pig||(pg===pls[i3].pig&&pls[pi].name<pls[i3].name)))){i3=pi;}}
    c1[i1]++;if(i2>=0)c2[i2]++;if(i3>=0)c3[i3]++;
  }
  var elim2={};Object.keys(koW).forEach(function(nr){var fx=koF[nr];if(!fx)return;var w=koW[nr];[fx[0],fx[1]].forEach(function(t){if(t&&t!==w)elim2[t]=1;});});
  var aliveT=function(t){return !!t&&inR32.indexOf(t)>=0&&!elim2[t];};
  var RM=[["r16",3],["qf",5],["sf",7],["final",8]];
  var remOf=function(ref){var s=0;RM.forEach(function(x){s+=(ref[x[0]]||[]).filter(function(t){return aliveT(t)&&(RESULTS[x[0]]||[]).indexOf(t)<0;}).length*x[1];});if(!RESULTS.champion&&aliveT(ref.champion))s+=10;return s;};
  var rows=pls.map(function(p,idx){return {name:p.name,cur:p.total,max:p.total+remOf(PLIST[idx].ref),p1:c1[idx]/N,p2:c2[idx]/N,p3:c3[idx]/N,top3:(c1[idx]+c2[idx]+c3[idx])/N};});
  rows.sort(function(a,b){return b.top3-a.top3||b.p1-a.p1||b.cur-a.cur||a.name.localeCompare(b.name);});
  _trKey=key;_trRows=rows;return rows;
}
function renderTitleRace(){var card=document.getElementById("titleRaceCard"),el=document.getElementById("titleRace"),sub=document.getElementById("trSub");if(!card||!el)return;
  if(!trOpen){card.style.display="none";return;}
  var rows=computeTitleRace();if(!rows){card.style.display="none";return;}card.style.display="";
  var pct=function(x){return (x*100).toFixed(2).replace(".",",")+"%";};
  var maxCeil=rows.reduce(function(m,r){return r.max>m?r.max:m;},1);
  if(sub)sub.textContent=rows.length?("· "+rows[0].name.split(" ")[0]+" "+pct(rows[0].top3)+" på pallen"):"";
  var head='<div class="tr-head"><span class="tr-rank"></span><span class="tr-name"></span><span class="tr-m">🥇</span><span class="tr-m">🥈</span><span class="tr-m">🥉</span></div>';
  var html=head+rows.map(function(r,i){var cw=Math.round(r.cur/maxCeil*100),pw=Math.round((r.max-r.cur)/maxCeil*100);
    return '<div class="tr-row'+(r.name===ME?" me":"")+(r.top3<=0?" tr-dead":"")+'"><div class="tr-main"><span class="tr-rank">'+(i+1)+'</span><span class="tr-nw"><span class="tr-nline"><span class="tr-name">'+r.name+'</span>'+(r.name===ME?'<span class="dut">DU</span>':"")+'</span></span><span class="tr-m">'+pct(r.p1)+'</span><span class="tr-m">'+pct(r.p2)+'</span><span class="tr-m">'+pct(r.p3)+'</span></div><div class="tr-barrow"><span class="tr-nowp">'+r.cur+' p</span><div class="tr-bar"><span class="tr-cur" style="width:'+cw+'%"></span><span class="tr-pot" style="width:'+pw+'%"></span></div><span class="tr-maxp">'+r.max+' p</span></div></div>';}).join("");
  html+='<div class="tr-src">Sannolikhet för 🥇/🥈/🥉-placering (topp 3 delar på potten), beräknad från 50 000 simulerade slutspel. Stapeln visar nuvarande poäng; talet till höger är max möjliga.</div>';
  el.innerHTML=html;}
function renderRank(){
  ...
  const ks=koState();
  let posMap=null;if(rankSort==="pig"||rankSort==="pis"){const srt=PLIST.slice().sort((a,b)=>(b[rankSort]-a[rankSort])||(b.total-a.total)||a.name.localeCompare(b.name));posMap={};let lastV=null,lastPos=0;srt.forEach((q,idx)=>{const v=q[rankSort];if(lastV===null||v!==lastV){lastPos=idx+1;lastV=v;}posMap[q.name]=lastPos;});}
  const subLine=p=>(ks.started&&rankSort!=="pig")?`<span class="t-ru">${(p.ref.r16||[]).filter(ks.alive).length} av 16 lag kvar</span> <span class="t-ex">· max ${p.total+koRemain(p.ref,ks)} p</span>`:`<span class="t-ru">${p.signs} rätt tecken</span> <span class="t-ex">· varav ${p.exact} exakt</span>`;
  ...
  var _maxK=null,_lastNrs=[],_lastGroup=false;MATCHES.forEach(function(m){if(!m.score)return;var k=parseKick(m.time);if(!k)return;var kt=k.getTime();if(_maxK===null||kt>_maxK){_maxK=kt;_lastNrs=[m.nr];_lastGroup=(m.t==="group");}else if(kt===_maxK){_lastNrs.push(m.nr);if(m.t!=="group")_lastGroup=false;}});
  const exactLatest=p=>_lastGroup&&_lastNrs.some(function(nr){return p.ref.groups.some(function(g){return g[0]===nr&&gpoints([g[2],g[3],g[5]],RESULTS.group[nr])===4;});});
  el.innerHTML=banner+(rows.map((p,i)=>{const hot=hotPts(p)>=5;const pos=posMap?posMap[p.name]:p.rank;return `<div class="rk ${pos===1&&anyPts()?"top":""} ${p.name===ME?"me":""}" data-name="${p.name}" tabindex="0" style="animation:rise .4s var(--ease) ${i*.025}s both"><span class="pos"><span class="rkn">${pos===1&&anyPts()?"👑":pos}</span>${rankSort==="total"?trendHtml(p):""}</span><span class="ava" style="background:${colorFor(p.name)}">${initials(p.name)}</span><span class="who"><span class="nm">${p.name}${p.name===ME?' <span class="dut">DU</span>':""}</span><span class="pp">${subLine(p)}</span></span><span class="tot">${hot?'<span class="fire" title="5+ poäng idag">🔥</span>':(exactLatest(p)?'<span class="bullseye" title="Exakt resultat i senaste matchen">🎯</span>':"")}<span class="cn" data-c="${rankSort==="pig"?p.pig:rankSort==="pis"?p.pis:p.total}">0</span><span class="totu">poäng</span></span></div>`}).join("")||'<div class="hint" style="padding:8px 4px">Ingen deltagare matchar sökningen.</div>');
...
function renderMatchList(){
  ...
  const ng=MATCHES.findIndex(x=>!x.score);
  ...
  _list.forEach(({m,i})=>{if(mFilter!=="all"&&m.t!==mFilter)return;
    ...
    let home,away,sc;
    if(m.home||m.away){home=`<div class="mx-team h">${m.home?flag(m.home,18):""}<span class="tn">${m.home||m.homeSlot}</span></div>`;away=`<div class="mx-team a"><span class="tn">${m.away||m.awaySlot}</span>${m.away?flag(m.away,18):""}</div>`;sc=m.score?`<div class="mx-score done">${m.score[0]}–${m.score[1]}</div>`:`<div class="mx-score soon">${to||"–"}</div>`;}
    ...
    else if(m.t!=="group"&&me&&(m.home||m.away)){const kp=koPickTeam(me,m);if(kp&&kp.team){const w=(RESULTS.ko||{})[m.nr];const lbl=m.ph==="Final"?" som mästare":" vidare";if(w){const ok=w===kp.team;mxp=`<span class="mxp ${ok?"exact":"wrong"}">${kp.team}${lbl} ${ok?"✓":"✗"}</span>`;guess=`<div class="mx-guess kog ${ok?"exact":"wrong"}"><span class="gl">Din gissning</span><span class="gv">${flag(kp.team,13)} ${kp.team}${lbl}</span><span class="go">${ok?"Rätt – laget gick vidare":"Fel – laget åkte ut"}</span></div>`;}else{mxp=`<span class="mxp pend">Du: ${kp.team}${lbl}</span>`;guess=`<div class="mx-guess kog pending"><span class="gl">Din gissning</span><span class="gv">${flag(kp.team,13)} ${kp.team}${lbl}</span></div>`;}}else if(kp&&!kp.team){mxp=`<span class="mxp pend">Du: ingen favorit</span>`;}}
...
function isLive(m){if(m.score)return false;if(LIVE&&LIVE[m.nr])return true;var k=parseKick(m.time);if(k){var dd=Date.now()-k;return dd>=0&&dd<9000000;}return false;}
function renderHero(){const el=document.getElementById("mxHero");if(!el)return;if(mSeg!=="upcoming"){el.innerHTML="";if(window._hcd)clearInterval(window._hcd);return;}const idx=MATCHES.findIndex(m=>!m.score);if(idx<0){el.innerHTML="";if(window._hcd)clearInterval(window._hcd);return;}const m=MATCHES[idx],isG=m.t==="group";var _me=ME?D.participants.find(p=>p.name===ME):null,_pk=(isG&&_me&&_me._g[m.nr])?_me._g[m.nr]:null;var _koG="";if(!isG&&_me&&(m.home||m.away)){var _kp=koPickTeam(_me,m);if(_kp&&_kp.team){var _kw=(RESULTS.ko||{})[m.nr];_koG='<div class="mxh-gwrap"><span class="mxh-guess kog'+(_kw?(_kw===_kp.team?" rt":" wr"):"")+'">Du tippade <b>'+_kp.team+(m.ph==="Final"?" som mästare":" vidare")+'</b>'+(_kw?(_kw===_kp.team?" ✓":" ✗"):"")+'</span></div>';}}var _live=isLive(m);const teams=(m.home||m.away)?`<div class="team"><div>${flagRound(m.home||"?",72)}</div><span class="tn">${m.home||m.homeSlot}</span></div><div class="sb"><div class="sc pend">VS</div><div class="ft">${isG?"Kommande":m.ph}</div></div><div class="team"><div>${flagRound(m.away||"?",72)}</div><span class="tn">${m.away||m.awaySlot}</span></div>`:`<div class="team"><div>${flagRound("?",72)}</div><span class="tn">${m.homeSlot||"TBD"}</span></div><div class="sb"><div class="sc pend">VS</div><div class="ft">Slutspel</div></div><div class="team"><div>${flagRound("?",72)}</div><span class="tn">${m.awaySlot||"TBD"}</span></div>`;el.innerHTML=`<div class="card fcard mxhero" data-i="${idx}"><div class="mxh-top"><span class="mxh-meta">📅 ${fmtMatchDate(m.time)||"TBD"}</span><span class="mxh-eyebrow${_live?" live":""}"><span class="mxh-dot"></span>${_live?"Pågår":"Nästa match"}</span></div><div class="mxh-cd" id="mxHeroCd"></div><div class="fbody">${teams}</div>${_pk?`<div class="mxh-gwrap"><span class="mxh-guess">Din gissning <b>${_pk[0]}–${_pk[1]}</b></span></div>`:_koG}</div>`;const hb=el.querySelector(".mxhero");if(hb)hb.onclick=()=>openMatch(idx);const cd=document.getElementById("mxHeroCd"),k=parseKick(m.time);if(window._hcd)clearInterval(window._hcd);if(cd&&_live){cd.innerHTML='<div class="mxh-live"><span class="mxh-livedot"></span>Inväntar resultat</div>';}else if(cd&&k){const _seg=function(n,l){return '<span class="cdu"><span class="cdn">'+("0"+n).slice(-2)+'</span><span class="cdt">'+l+'</span></span>';};const tick=()=>{const d=k-Date.now();if(d<=0){cd.innerHTML='<div class="cdlive">🟢 Avspark passerad – inväntar resultat</div>';if(window._hcd)clearInterval(window._hcd);return;}var dd=Math.floor(d/864e5),hh=Math.floor(d%864e5/36e5),mm=Math.floor(d%36e5/6e4),ss=Math.floor(d%6e4/1e3);cd.innerHTML='<div class="cdtop">Avspark om</div><div class="cdrow">'+(dd>0?_seg(dd,"DGR")+'<span class="cdsep">:</span>':"")+_seg(hh,"TIM")+'<span class="cdsep">:</span>'+_seg(mm,"MIN")+'<span class="cdsep">:</span>'+_seg(ss,"SEK")+'</div>';};tick();window._hcd=setInterval(tick,1000);}else if(cd){cd.textContent="";}}
...
function renderBracket(){
  ...
  const rp={r32:2,r16:3,qf:5,sf:7,final:8}[key];
  const entry=(team,slot,win)=>{const mine=team&&mineSet.includes(team);return `<div class="brow ${win?"win":""} ${mine?"mine":""}">${team?`${flag(team,14)} <span class="bt">${team}</span>`:`<span class="slot">${slot}</span>`}${mine?'<span class="chk">✓ +'+rp+' p</span>':""}</div>`;};
  const ms=D.knockout.filter(k=>k.round===rd);
  const scoreByNr={};MATCHES.forEach(function(mm){if(mm.t==="ko"&&mm.score)scoreByNr[mm.nr]=mm.score;});
  document.getElementById("bracketWrap").innerHTML=ms.map(k=>{const fx=kf[k.nr],w=kw[k.nr];const h=fx?fx[0]:null,a=fx?fx[1]:null;
    return `<div class="kocard"><div class="kometa">match #${k.nr} \u00b7 ${k.date}${scoreByNr[k.nr]?` · <span class="kosc">${scoreByNr[k.nr][0]}–${scoreByNr[k.nr][1]}</span>`:""}</div>${entry(h,k.homeSlot,w&&h===w)}${entry(a,k.awaySlot,w&&a===w)}</div>`;}).join("");
  const _kn=document.getElementById("koNote");if(_kn)_kn.innerHTML=(!Object.keys(RESULTS.koFixtures||{}).length)?'<div class="emptynote">⚽ Slutspelet börjar 28 juni – lagen och poängen fylls i automatiskt när gruppspelet är avgjort.</div>':(me?'<div class="kolegend">✓ = lag du tippade till denna rond</div>':"");renderKoScore(me);
}
function koJourneyHtml(ref,isMe){
  var ks=koState();
  var aliveChip=function(t){return t?(ks.alive(t)?'<span class="ksst alive">kvar</span>':'<span class="ksst out">utslagen</span>'):"";};
  var R=[["Lag till 16-delsfinal","r32",2],["Lag till åttondelsfinal","r16",3],["Kvartsfinallag","qf",5],["Semifinallag","sf",7],["Finallag","final",8]];
  var tot=0;
  var rows=R.map(function(x){var l=x[0],k=x[1],p=x[2],mine=ref[k]||[];var got=mine.filter(function(t){return (RESULTS[k]||[]).indexOf(t)>=0;}).length;var poss=mine.filter(function(t){return ks.alive(t)&&(RESULTS[k]||[]).indexOf(t)<0;}).length;tot+=got*p;return '<div class="ksrow"><span>'+l+'</span><span class="ksv">'+got+' rätt × '+p+'p'+(poss?' · '+poss+' kvar':'')+'</span><span class="ksp">'+(got*p)+'</span></div>';}).join("");
  var remain=koRemain(ref,ks);
  var champScored=(RESULTS.champion&&ref.champion===RESULTS.champion)?10:0;tot+=champScored;
  var champChip=RESULTS.champion?(ref.champion===RESULTS.champion?'<span class="ksst win">mästare! +10p</span>':'<span class="ksst out">fel</span>'):aliveChip(ref.champion);
  var finItems=(ref.final||[]).map(function(t){var c;if((RESULTS.final||[]).indexOf(t)>=0){c=(RESULTS.champion===t)?'<span class="ksst win">världsmästare!</span>':(ks.elim[t]?'<span class="ksst out">förlorade finalen</span>':'<span class="ksst win">i final</span>');}else{c=aliveChip(t);}return flag(t,14)+' '+t+' '+c;}).join(' <span class="ksr-sep">·</span> ');
  return '<div class="ksbox">'
    +'<div class="kstot"><span>'+(isMe?'Din slutspelspoäng':'Slutspelspoäng')+'</span><b>'+tot+' p</b></div>'
    +(remain>0?'<div class="ksremain">🎯 Kvar att spela om: <b>'+remain+' p</b></div>':'')
    +'<div class="ksresa"><div class="ksr-item"><span class="ksr-l">👑 Mästare</span><span class="ksr-v">'+(ref.champion?flag(ref.champion,14)+' '+ref.champion+' '+champChip:'–')+'</span></div>'
    +'<div class="ksr-item"><span class="ksr-l">🥇 Finalister</span><span class="ksr-v">'+(finItems||'–')+'</span></div></div>'
    +rows
    +'<div class="ksrow"><span>Världsmästare</span><span class="ksv">'+(RESULTS.champion?(ref.champion===RESULTS.champion?"rätt":"fel"):"ej avgjort")+'</span><span class="ksp">'+champScored+'</span></div>'
    +'</div>';
}
function renderKoScore(me){const el=document.getElementById("koScore");if(!el)return;
  if(!me){el.innerHTML='<div class="hint" style="margin-bottom:12px">⭐ Välj ditt namn där uppe för att se din slutspelsresa här.</div>';return;}
  el.innerHTML=koJourneyHtml(me,true);}
...
function renderTables(){document.getElementById("tablesWrap").innerHTML=Object.keys(D.groups).sort().map(L=>{const rows=groupTable(L);
  return `<div class="gtab"><h4>Grupp ${L}</h4><table><thead><tr><th>Lag</th><th>S</th><th>V</th><th>O</th><th>F</th><th>GM</th><th>IM</th><th class="gd">MS</th><th>P</th></tr></thead><tbody>${rows.map((r,i)=>`<tr class="${i<2?'qual':''}"><td style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${flag(r[0],13)}${r[0]}</td><td>${r[1].P}</td><td>${r[1].W}</td><td>${r[1].D}</td><td>${r[1].L}</td><td>${r[1].GF}</td><td>${r[1].GA}</td><td class="gd ${r[1].GF-r[1].GA>0?'pos':r[1].GF-r[1].GA<0?'neg':''}">${r[1].GF-r[1].GA>0?"+"+(r[1].GF-r[1].GA):r[1].GF-r[1].GA}</td><td><b>${r[1].Pts}</b></td></tr>`).join("")}</tbody></table></div>`;}).join("");renderBestThirds();}
function renderBestThirds(){var el=document.getElementById("bestThirds");if(!el)return;
  var thirds=Object.keys(D.groups).sort().map(function(L){var r=groupTable(L)[2];return r?{grp:L,team:r[0],s:r[1]}:null;}).filter(Boolean);
  thirds.sort(function(a,b){return b.s.Pts-a.s.Pts||(b.s.GF-b.s.GA)-(a.s.GF-a.s.GA)||b.s.GF-a.s.GF||a.team.localeCompare(b.team);});
  var html="";thirds.forEach(function(t,i){var gd=t.s.GF-t.s.GA;html+='<div class="bt-row '+(i<8?"q":"out")+'"><span class="bt-rank">'+(i+1)+'</span><span class="bt-grp">'+t.grp+'</span>'+flag(t.team,14)+'<span class="bt-team">'+t.team+'</span><span class="bt-stat">'+t.s.Pts+' p</span><span class="bt-stat bt-gd">'+(gd>0?"+":"")+gd+'</span></div>';if(i===7)html+='<div class="bt-cut">Slutspelsgräns</div>';});
  el.innerHTML=html;}
function fillSelects(){const a=document.getElementById("h2hA"),b=document.getElementById("h2hB");const o=PLIST.map(p=>`<option>${p.name}</option>`).join("");a.innerHTML=o;b.innerHTML=o;var _mi=ME?PLIST.findIndex(p=>p.name===ME):-1;a.selectedIndex=_mi>=0?_mi:0;b.selectedIndex=a.selectedIndex===0?Math.min(1,PLIST.length-1):0;a.onchange=b.onchange=renderH2H;renderH2H();}
...
function kboxHtml(pr){var fin=(pr.final&&pr.final.length)?pr.final.map(function(t){return flag(t,13)+" "+t;}).join(", "):"–";return `<div class="kbox"><b>🥇 Final-tips:</b> ${fin}<br><b>🏆 Världsmästare:</b> ${pr.champion?flag(pr.champion,14)+" "+pr.champion:"–"}<br><b>👟 Skyttekung:</b> ${pr.skyttekung||"–"}${pr.goals?` (${pr.goals} mål)`:""}</div>`;}
function renderH2H(){
  if(!document.getElementById("h2hA")||!document.getElementById("h2hB"))return;
...
function openPerson(name){const p=PLIST.find(x=>x.name===name),pr=p.ref;
  ...
  document.getElementById("mBody").innerHTML=`<div class="statrow"><div class="stat"><div class="v">${p.pig} <span class="vp">poäng</span></div><div class="l">Gruppspel</div></div><div class="stat"><div class="v">${p.pis} <span class="vp">poäng</span></div><div class="l">Slutspel</div></div><div class="stat"><div class="v">${p.total} <span class="vp">poäng</span></div><div class="l">Total</div></div></div>${kboxHtml(pr)}${koJourneyHtml(pr,false)}<h4 style="margin:14px 0 6px">🎯 Gruppspel <span class="gpsub" style="font-weight:500;color:var(--muted);font-size:12px">(${g.exact+g.right} rätt tecken, varav ${g.exact} exakt \u00b7 ${g.gained} p på ${g.played} spelade)</span></h4><div class="psheet">${g.rows}</div>`;
  document.getElementById("overlay").classList.add("open");
}
...
const tb=document.getElementById("themeBtn");if(tb){var _SUN='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/></svg>',_MOON='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8z"/></svg>';function _setIco(){tb.innerHTML=document.documentElement.getAttribute("data-theme")==="dark"?_SUN:_MOON;}_setIco();tb.onclick=()=>{var d=document.documentElement.getAttribute("data-theme")==="dark",nt=d?"light":"dark";document.documentElement.setAttribute("data-theme",nt);_setIco();var mc=document.querySelector('meta[name="theme-color"]');if(mc)mc.setAttribute("content",nt==="dark"?"#070b1a":"#eef2fb");try{localStorage.setItem("loftahammar_theme",nt);}catch(e){}};}
fillMe();renderMeHem();renderPodium();renderFeatured();renderRank();renderMatchList();renderBracket();renderTables();renderChampViz();renderUpcoming();fillSelects();setupRankControls();renderTitleRace();(function(){var ic=document.querySelector("#standings .ttl .ic");if(!ic)return;var taps=0,tmr=null;ic.addEventListener("click",function(){taps++;clearTimeout(tmr);tmr=setTimeout(function(){taps=0;},900);if(taps>=3){taps=0;trOpen=!trOpen;renderTitleRace();if(trOpen){var c=document.getElementById("titleRaceCard");if(c)c.scrollIntoView({block:"nearest",behavior:"smooth"});}}});})();(function(){const _pi=document.getElementById("poengInfo");if(_pi)_pi.onclick=openScoring;})();(function(){var _ts=document.getElementById("toStandings");if(_ts)_ts.onclick=function(){goto("standings");};})();(function(){const _h=location.hash.slice(1);if(_h&&document.getElementById(_h))goto(_h,true);})();
(function(){var me=document.querySelector(".hdr-me"),bar=document.querySelector(".mebar"),foot=document.querySelector(".hdr-foot");if(!me||!bar||!foot)return;var mq=window.matchMedia("(max-width:560px)");function place(){if(mq.matches){if(me.parentNode!==bar)bar.appendChild(me);bar.style.display="flex";}else{if(me.parentNode!==foot)foot.appendChild(me);bar.style.display="none";}}place();try{mq.addEventListener("change",place);}catch(e){mq.addListener(place);}})();
(function(){
  ...
  function run(){
    ...
    function rest(){document.documentElement.style.scrollBehavior=prevSB;var last=window.scrollY,fired=false;clone.style.pointerEvents="auto";clone.style.cursor="pointer";function trigger(){if(fired)return;fired=true;window.removeEventListener("scroll",onS);clone.removeEventListener("click",onClick);clone.style.pointerEvents="none";rollOff();}function onS(){var y=window.scrollY;if(y<last-2)trigger();last=y;}function onClick(){trigger();}window.addEventListener("scroll",onS,{passive:true});clone.addEventListener("click",onClick);}
  ...
  ball.addEventListener("click",function(){if(mqRm.matches||running)return;run();});
})();
...
function mapFeed(j){if(!j||!j.matches)return null;
var alias={"Bosnia-Herzegovina":"Bosnien","Bosnien och Hercegovina":"Bosnien","Cape Verde Islands":"Kap Verde","Curaçao":"Curacao","Congo DR":"DR Kongo","Ecuador":"Equador"};
var nm=function(t){return t?(alias[t]||t):null;};
var fx={};D.fixtures.forEach(function(f){fx[f.grp+"|"+f.home+"|"+f.away]=f.nr;});
var koRound={LAST_32:"16-delsfinal",LAST_16:"Åttondelsfinal",QUARTER_FINALS:"Kvartsfinal",SEMI_FINALS:"Semifinal",FINAL:"Final"};
var advKey={LAST_32:"r16",LAST_16:"qf",QUARTER_FINALS:"sf",SEMI_FINALS:"final"};
var out={},live={},koFixtures={},ko={},koScore={},koTime={},champion=null,rs={r32:{},r16:{},qf:{},sf:{},final:{}};
var svT=function(iso){var d=new Date(iso);if(isNaN(d))return null;d=new Date(d.getTime()+7200000);var p=function(n){return (n<10?"0":"")+n;};return p(d.getUTCDate())+"/"+p(d.getUTCMonth()+1)+" "+p(d.getUTCHours())+"."+p(d.getUTCMinutes());};
j.matches.forEach(function(m){if(m.stage!=="GROUP_STAGE")return;var hn=nm(m.homeTeam),an=nm(m.awayTeam);var g=(m.group||"").replace("GROUP_","");var nr=fx[g+"|"+hn+"|"+an];if(nr==null)return;if(m.status==="IN_PLAY"||m.status==="PAUSED"){live[nr]=true;return;}if(m.status==="FINISHED"&&m.homeScore!=null&&m.awayScore!=null)out[nr]=[m.homeScore,m.awayScore];});
/* Slutspel: para feedens matcher med våra matchnummer per rond i KRONOLOGISK ORDNING,
   inte via exakt klockslag – feeden kan justera avsparkstider (hände med match 79). */
var dsR={};D.knockout.forEach(function(k){(dsR[k.round]=dsR[k.round]||[]).push(k);});
Object.keys(koRound).forEach(function(st){
  var fm=j.matches.filter(function(m){return m.stage===st;}).sort(function(a,b){return a.utcDate<b.utcDate?-1:a.utcDate>b.utcDate?1:0;});
  var dm=(dsR[koRound[st]]||[]).slice().sort(function(a,b){return parseKick(a.date)-parseKick(b.date);});
  var n=Math.min(fm.length,dm.length);
  for(var q=0;q<n;q++){var m=fm[q],knr=dm[q].nr,hn=nm(m.homeTeam),an=nm(m.awayTeam);
    var tv=svT(m.utcDate);if(tv)koTime[knr]=tv;
    if(hn||an)koFixtures[knr]=[hn,an];
    if(st==="LAST_32"){if(hn)rs.r32[hn]=1;if(an)rs.r32[an]=1;}
    if(m.status==="IN_PLAY"||m.status==="PAUSED")live[knr]=true;
    if(m.status==="FINISHED"){if(m.homeScore!=null&&m.awayScore!=null)koScore[knr]=[m.homeScore,m.awayScore];
      if(m.winner==="HOME_TEAM"||m.winner==="AWAY_TEAM"){var w=m.winner==="HOME_TEAM"?hn:an;if(w){ko[knr]=w;if(advKey[st])rs[advKey[st]][w]=1;else if(st==="FINAL")champion=w;}}}}
});
return {group:out,updated:j.updatedAt||null,live:live,koFixtures:koFixtures,ko:ko,koScore:koScore,koTime:koTime,r32:Object.keys(rs.r32),r16:Object.keys(rs.r16),qf:Object.keys(rs.qf),sf:Object.keys(rs.sf),final:Object.keys(rs.final),champion:champion};}
function applyResults(res){if(!res||!res.group)return;LIVE=res.live||{};Object.assign(RESULTS.group,res.group);if(res.koFixtures&&Object.keys(res.koFixtures).length)RESULTS.koFixtures=res.koFixtures;if(res.ko&&Object.keys(res.ko).length)RESULTS.ko=res.ko;["r32","r16","qf","sf","final"].forEach(function(k){if(res[k]&&res[k].length)RESULTS[k]=res[k];});if(res.champion)RESULTS.champion=res.champion;PLIST=buildPlist();MATCHES.forEach(function(m){if(m.t==="group"){m.score=RESULTS.group[m.nr]||null;}else{var _fx=RESULTS.koFixtures&&RESULTS.koFixtures[m.nr];m.home=(_fx&&_fx[0])||null;m.away=(_fx&&_fx[1])||null;m.score=(res.koScore&&res.koScore[m.nr])||null;if(res.koTime&&res.koTime[m.nr])m.time=res.koTime[m.nr];}});if(res.koTime)D.knockout.forEach(function(k){if(res.koTime[k.nr])k.date=res.koTime[k.nr];});cur=MATCHES.findIndex(function(m){return !m.score;});if(cur<0)cur=0;if(res.updated){var _f=fmtUpd(res.updated),_eu=document.getElementById("updatedAt");if(_f&&_eu)_eu.innerHTML="Senast uppdaterad "+_f;}renderPodium();renderFeatured();renderRank();renderMatchList();renderTables();renderChampViz();renderUpcoming();renderMeHem();renderH2H();renderBracket();renderTitleRace();}
